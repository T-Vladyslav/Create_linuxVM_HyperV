...
# Creating a list of the latest hosts for each unique name
$lastHosts = @{}
foreach ($vm in $vmParams) {
    $lastHosts[$vm.'Parent Host Name'] = $vm
}
...
foreach ($vm in $vmParams) {
    Invoke-Command -ComputerName $vm.'Parent Host Name' -Credential $credential -ScriptBlock {
        ...
        if ($clusterVm) {
            ...
            ## Checking if the current host is the last one for its unique name
            $lastVm = $lastHosts[$vm.'Parent Host Name']
            ...
            if ($vm.'VM Name' -eq $lastVm.'VM Name') {
                ...
                $taskEnd = 30 # time of task life in seconds
                
                $taskAction = New-ScheduledTaskAction -Execute "powershell.exe" -Argument '-NoProfile -WindowStyle Hidden -Command "& {$currentTime = Get-Date; $timeAgo = $currentTime.AddHours(-1); $recentVMs = Get-VM | Where-Object { $_.CreationTime -gt $timeAgo }; foreach ($vm in $recentVMs) { Add-ClusterVirtualMachineRole -VMName $vm.Name}}"'
